...
function apply() {
	...
	for ( i = 0; i < nargs; i++ ) {
		x = args[ i ];
		if ( !isMatrixLike( x ) ) {
			throw new TypeError( 'invalid input argument. Input data structures must be matrices. Value: `' + x + '`.' );
		}
		if ( i === 0 ) {
			M = x.shape[ 0 ];
			N = x.shape[ 1 ];
	...
	if ( nargs <= 0 ) {
		throw new Error( 'insufficient input arguments. Must provide input matrices.' );
	}
	arr = new Array( nargs );
	for ( i = 0; i < M; i++ ) {
		for ( j = 0; j < N; j++ ) {
