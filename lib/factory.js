...
function factory() {
	...
	if ( !isNumber( num ) ) {
		throw new TypeError( 'apply()::invalid input argument. Argument specifying number of input matrices must be a number primitive. Value: `' + num + '`.' );
	}
	// If an apply function has been provided, validate...
	if ( vFLG === 123 ) {
		if ( !isFunction( fcn ) ) {
			throw new TypeError( 'apply()::invalid input argument. Apply function must be a function. Value: `' + fcn + '`.' );
		}
	}
	...
	*/
	return function apply() {
		...
		if ( (nargs-k) !== num ) {
			throw new Error( 'apply()::insufficient input arguments. Must provide ' + num + ' input matrices.' );
		}
		for ( i = k; i < nargs; i++ ) {
